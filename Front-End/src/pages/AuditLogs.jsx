          ...
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50 text-left">
              {['Time', 'User', 'Module', 'Action', 'IP Address'].map((h) => (
                <th key={h} className="px-6 py-3.5 font-semibold text-gray-600 text-xs uppercase tracking-wide">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {logs.length === 0 && <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-400">No logs found</td></tr>}
            {logs.map((log) => (
